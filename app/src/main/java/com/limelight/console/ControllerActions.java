...
/** Controller actions ported as an isolated module from Wake & Play. */
final class ControllerActions {
    private static final String TAG = "MoonlightConsole";
    ...
    static void identify(int deviceId, Handler handler, ResultCallback callback) {
        ...
        if (vibrationStarted || lightsStarted) {
            callback.onResult(true, lightsStarted
                    ? "The controller is flashing and vibrating."
                    : "The controller is vibrating.");
        } else {
            ...
            for (int step = 0; step < colors.length; step++) {
                ...
                handler.postDelayed(() -> {
                    try {
                        ...
                        Log.w(TAG, "Controller LED flash failed", error);
                    }
                }, step * 220L);
            }
            handler.postDelayed(() -> {
    ...
    static void disconnect(Context context, int deviceId, ResultCallback callback) {
        ...
        try {
            ...
            callback.onResult(success, success
                    ...
                    : "The system rejected the power-off request.");
            return;
        } catch (NoSuchMethodException ignored) {
            // Older Android releases expose this only through the HID Host profile.
        ...
        boolean requested = adapter.getProfileProxy(context, new BluetoothProfile.ServiceListener() {
            @Override public void onServiceConnected(int profile, BluetoothProfile proxy) {
                ...
                callback.onResult(success, success
                        ? "The controller was powered off."
                        : "Android blocked the controller power-off request." + suffix(detail));
            }
    ...
    static void unpair(int deviceId, ResultCallback callback) {
        ...
        try {
            ...
            boolean success = !(value instanceof Boolean) || (Boolean) value;
            callback.onResult(success, success
                    ? "Controller unpairing started."
                    : "Android rejected the unpair request.");
        } catch (Exception error) {
            Log.w(TAG, "Bluetooth removeBond blocked", error);
            callback.onResult(false, "Android blocked controller unpairing." + suffix(rootMessage(error)));
        }
    }
    ...
    private static BluetoothDevice resolveBluetoothDevice(InputDevice input) {
        ...
        if (descriptor != null) {
            for (BluetoothDevice candidate : bonded) {
                if (descriptor.equals(descriptorForBluetoothDevice(input, candidate))) return candidate;
            }
        }
    ...
    private static String descriptorForBluetoothDevice(InputDevice input, BluetoothDevice device) {
        try {
            ...
            Log.w(TAG, "Unable to derive Bluetooth input descriptor", error);
            return null;
        }
    }
